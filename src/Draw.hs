...
drawCircle s (x, y) r c =
  mapM_ (draw . fromIntegral) [0, 1..359] >> return ()
  where
    tr :: Double
    tr = fromIntegral r
    draw :: Double -> IO Bool
...
drawBC state bcn =  do
  ...
  when (bcntype bcn == VOR) $ do
    ...
    GFX.polygon sc (map (\(a, b) -> (fromIntegral a, fromIntegral b))
                    [(bcnX-10, bcnY+5),
                     (bcnX-10, bcnY-5),
                     (bcnX-5, bcnY-10),
                     (bcnX+5, bcnY-10),
...
drawWP s (VFRRP lon lat comp dsg dsgl ctr) = do
  case comp of
    True -> drawPicture s coords "crp"
    False -> drawPicture s coords "orrp"
